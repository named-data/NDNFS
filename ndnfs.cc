...
ndnfs_getattr(const char *path, struct stat *stbuf)
{
    ...
    if (type == dir_type) {
        ...
        stbuf->st_nlink = 1;
        stbuf->st_size = strlen(entry.getStringField("data"));  //TODO: add file size as a field in BSON document
    }
...
ndnfs_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if ((fi->flags & O_ACCMODE) != O_RDONLY) //TODO: check mode
        return -EACCES;
...
ndnfs_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    ...
    auto_ptr<DBClientCursor> cursor = c.query(db_name, QUERY("name" << "/"));
    if (!cursor->more())
        return -ENOENT;
    
    string abs_path(path);
    vector< string > path_components;
    split(path_components, abs_path, is_any_of("/")); // the first component is an empty string
    
    vector< BSONElement > dir_data = cursor->next()["data"].Array();
    
    // Build new file entry with empty content
    BSONObj file_entry = BSONObjBuilder().append("name", path_components[1].c_str()).append("type", "file").append("data", "").obj();
    // Append to existing BSON array  
    c.update(db_name, BSON("name" << "/"), BSON( "$push" << BSON( "data" << file_entry ) ));
